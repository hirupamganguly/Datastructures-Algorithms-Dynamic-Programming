public class MaxBinaryHeap {
    ...
    String ins(int item){
        ...
        while(newNode>0 && temp>heap[parent(newNode)]){ 
            //untill newly added item temp is lesser than parent of newnode variable loop:-
    ...
    int maxChild(int ind){
        ...
        if(rightChild<heapSize){
            // if rightchild index is not lesser than heapsize
    ...
    String del(int index){
        ...
        while(kthChild(index, 1)< heapSize){
            //untill left child of index is lesser than heapsize loop-
            child=maxChild(index);//index of min child- of index
            if (heap[child]> temp1) heap[index]=heap[child];// if the min child element is greater than temp1 which contain the puted item
